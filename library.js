...
const formatSpoiler = (text) => {
	...
	return text.replace(startSpoiler, (match, p1) => `<div class="spoiler-wrapper"><button class="spoiler-control btn btn-default">${p1 ? p1 : 'Spoiler'}</button><div style="display: none;" class="spoiler-content">`).replace(endSpoiler, (match, p1) => `${p1}</div></div>`);
}
